...
DEFAULT_CLOSE_REASON = bytes('I Love You', encoding='utf-8')
...
class WebsocketServer(ThreadingMixIn, TCPServer, API):
    ...
    def __init__(self, host='127.0.0.1', port=0, loglevel=logging.WARNING, cert=None, key=None, password=None):
...
class WebSocketHandler(StreamRequestHandler):
    ...
    def __init__(self, socket, addr, server):
        ...
        if server.cert and server.key:
            sslContext = ssl.SSLContext(ssl.PROTOCOL_TLS_SERVER)
            try:
                sslContext.load_cert_chain(certfile=server.cert, keyfile=server.key, password=server.password)
                socket = sslContext.wrap_socket(socket, server_side=True)
